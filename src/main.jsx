            ...
            <div className="w-full max-w-md p-8 space-y-8 bg-white rounded-xl shadow-lg">
                ...
                <form className="space-y-6" onSubmit={handleLogin}>
                    <div className="relative">
                        <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} className="w-full px-4 py-3 text-gray-700 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="Email" required />
                    </div>
                    <div className="relative">
                        <input type="password" value={password} onChange={(e) => setPassword(e.target.value)} className="w-full px-4 py-3 text-gray-700 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="Senha" required />
                    </div>
  ...
  const loadAllData = async () => {
    ...
    try {
        ...
        const [clientsData, vehiclesData, servicesData, budgetsData, expensesData] = await Promise.all([
            fetchCollection('clients'), fetchCollection('vehicles'), fetchCollection('services'), fetchCollection('budgets'), fetchCollection('expenses'),
        ]);
...
function Dashboard({ clients, vehicles, services, budgets }) {
    ...
    return ( <div className="animate-fade-in"> <h2 className="text-3xl font-bold mb-8 text-gray-800 dark:text-white">Dashboard</h2> <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8"> <StatCard title="Total de Clientes" value={clients.length} icon={<Users className="text-white" />} color="bg-blue-500" /> <StatCard title="Total de Veículos" value={vehicles.length} icon={<Car className="text-white" />} color="bg-green-500" /> <StatCard title="Serviços em Andamento" value={services.filter(s => s.status === 'Em andamento').length} icon={<Wrench className="text-white" />} color="bg-yellow-500" /> <StatCard title="Orçamentos Pendentes" value={budgets.filter(b => b.status === 'Pendente').length} icon={<ClipboardList className="text-white" />} color="bg-indigo-500" /> </div> </div> );
}
...
function Services({ services }) {
    ...
    const renderRow = (service) => ( <tr key={service.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50"> <td className="p-4 text-gray-800 dark:text-gray-200 font-bold">#{service.budgetId.substring(0,5)}</td> <td className="p-4 text-gray-600 dark:text-gray-300">{service.clientName}</td> <td className="p-4 text-gray-600 dark:text-gray-300">{service.vehicleModel}</td> <td className="p-4 text-gray-600 dark:text-gray-300">{service.date}</td> <td className="p-4 text-gray-600 dark:text-gray-300">R$ {service.totalPrice.toFixed(2)}</td> <td className="p-4"><span className={`px-3 py-1 text-sm font-medium rounded-full ${service.status === 'Concluído' ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'}`}>{service.status}</span></td> <td className="p-4 text-right"><button className="text-blue-500 hover:text-blue-700 mr-2">Detalhes</button></td> </tr> );
