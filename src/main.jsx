...
// (Você encontra isso no console do Firebase, nas configurações do seu projeto web)
// =================================================================================
...
function App() {
  ...
  const handleSave = async (type, data) => {
    setIsLoading(true);
    try {
        const { id, ...dataToSave } = data; // Remove ID para não salvar no documento
        if (id) {
  ...
  const renderView = () => {
    if (isLoading) return <div className="flex justify-center items-center h-full"><p className="text-xl">Carregando dados do sistema...</p></div>;
  ...
  const renderModal = () => {
    ...
    switch (modal.type) {
        ...
        case 'expense': return handleSave('expenses', modal.data);
        default: return null;
    }
  };
  ...
  return (
    <>
      <div className="flex h-screen bg-gray-100 dark:bg-gray-900 font-sans">
        ...
        <main className="flex-1 flex flex-col"><header className="bg-white dark:bg-gray-800 shadow-md p-4 flex justify-between items-center lg:justify-end z-30">
            <button className="lg:hidden text-gray-600 dark:text-gray-300" onClick={() => setIsMenuOpen(true)}><Menu size={28} /></button>
            <div className="flex items-center"><span className="mr-4 text-gray-700 dark:text-gray-200">Bem-vindo, Usuário!</span></div>
          </header>
    ...
    return (
        <Modal title={client ? 'Editar Cliente' : 'Novo Cliente'} onClose={onClose}>
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
                <div><label>Nome</label><input name="name" value={data.name} onChange={handleChange} className="w-full mt-1 p-2 border rounded" required /></div>
                <div><label>Telefone</label><input name="phone" value={data.phone} onChange={handleChange} className="w-full mt-1 p-2 border rounded" /></div>
                <div><label>Email</label><input type="email" name="email" value={data.email} onChange={handleChange} className="w-full mt-1 p-2 border rounded" /></div>
                <div className="pt-4 flex justify-end"><button type="submit" className="px-6 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700">Salvar</button></div>
            </form>
        </Modal>
    );
}
...
function VehicleEditor({ vehicle, clients, onSave, onClose }) {
    ...
    return (
        <Modal title={vehicle ? 'Editar Veículo' : 'Novo Veículo'} onClose={onClose}>
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
                <div><label>Cliente</label><select name="clientId" value={data.clientId} onChange={handleChange} className="w-full mt-1 p-2 border rounded" required><option value="">Selecione</option>{clients.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}</select></div>
                <div><label>Modelo</label><input name="model" value={data.model} onChange={handleChange} className="w-full mt-1 p-2 border rounded" required /></div>
                <div><label>Placa</label><input name="plate" value={data.plate} onChange={handleChange} className="w-full mt-1 p-2 border rounded" required /></div>
                <div><label>Ano</label><input name="year" value={data.year} onChange={handleChange} className="w-full mt-1 p-2 border rounded" /></div>
                <div><label>Cor</label><input name="color" value={data.color} onChange={handleChange} className="w-full mt-1 p-2 border rounded" /></div>
                <div><label>KM</label><input name="km" value={data.km} onChange={handleChange} className="w-full mt-1 p-2 border rounded" /></div>
                <div className="pt-4 flex justify-end"><button type="submit" className="px-6 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700">Salvar</button></div>
            </form>
        </Modal>
    );
...
function BudgetEditor({ budget, clients, vehicles, onSave, onClose }) {
  ...
  const generatePDF = () => {
    alert("A geração de PDF será configurada no ambiente de produção final.");
  };
  ...
  return (
    <Modal title={budget ? 'Editar Orçamento' : 'Novo Orçamento'} onClose={onClose}>
        <div className="p-6 overflow-y-auto" ref={budgetPDF}>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                <div><label>Cliente</label><select value={currentBudget.clientId} onChange={(e) => handleClientChange(e.target.value)} className="w-full mt-1 p-2 border rounded" required><option value="">Selecione</option>{clients.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}</select></div>
                <div><label>Veículo</label><select value={currentBudget.vehicleId} onChange={(e) => setCurrentBudget(prev => ({...prev, vehicleId: e.target.value}))} className="w-full mt-1 p-2 border rounded" disabled={!currentBudget.clientId} required><option value="">Selecione</option>{filteredVehicles.map(v => <option key={v.id} value={v.id}>{`${v.model} - ${v.plate}`}</option>)}</select></div>
            </div>
            <div className="space-y-2">
                {currentBudget.items.map(item => (
                    <div key={item.id} className="grid grid-cols-12 gap-2 items-center">
                        <input type="text" value={item.desc} onChange={e => handleItemChange(item.id, 'desc', e.target.value)} placeholder="Descrição" className="col-span-6 p-2 border rounded" />
                        <input type="number" value={item.qty} onChange={e => handleItemChange(item.id, 'qty', parseFloat(e.target.value) || 0)} className="col-span-1 p-2 border rounded" />
                        <input type="number" value={item.price} onChange={e => handleItemChange(item.id, 'price', parseFloat(e.target.value) || 0)} className="col-span-2 p-2 border rounded" />
        ...
        <div className="p-4 bg-gray-50 dark:bg-gray-900 border-t dark:border-gray-700 flex justify-end space-x-3">
            ...
            <button onClick={() => onSave(currentBudget)} className="px-6 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700">Salvar</button>
        </div>
    </Modal>
  );
}
